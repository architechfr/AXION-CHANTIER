...
├── cadence.html            ← page « Qui sommes-nous » Cadence (associés, équipe, compétences)
...
- **Maquette 3D** (`chantier.html`, onglet « Maquette 3D » — 10/06) : visionneuse `<model-viewer>` (Google, CDN) **lecture seule** — orbite/zoom/pincement, recentrer, rotation auto, plein écran. GLB optimisé web fourni par O. Pénarette (mail 10/06) servi par Vercel : `assets/3d/maquette-jardins-de-lisa.glb` (1,5 Mo). ⚠️ Le GLB est compressé `EXT_meshopt_compression` → décodeur Meshopt configuré via `meshoptDecoderLocation` (build **classique** `meshopt_decoder.js`, PAS le `.module.js`). Onglet visible **uniquement** si une maquette existe : mapping `MODELS_3D` par slug dans `chantier.html` (lit `project.model3d_url` en priorité si la colonne est créée un jour). Chargement différé : le GLB n'est téléchargé qu'à la 1ʳᵉ ouverture de l'onglet. Pour ajouter une maquette à un autre chantier : déposer le `.glb` (optimisé meshopt) dans `assets/3d/` + ajouter l'entrée au mapping. **Iris ajouté le 11/06** : `maquette-jardins-iris.glb` (6,9 Mo, 2,5 M triangles — mail O. Pénarette du 11/06 intitulé « Noisy » mais affecté à **Jardins d'Iris** sur instruction F. Clarisse ; fichier source `NOISY_web_optimisee.glb`). ⚠️ Modèle 5× plus lourd que Lisa : si l'affichage rame, demander à Oscar un export décimé. **Noisy ajouté le 11/06** : `maquette-villages-or-noisy.glb` (4,4 Mo, 1,5 M triangles — mail O. Pénarette du 11/06 « la vraie maquette de Noisy », fichier `noisy2_web_optimisee.glb`), slug `villages-or-noisy`. Zoom rapproché débridé sur la visionneuse (`min-camera-orbit` 1 m + `min-field-of-view` 10°) — demande F. Clarisse 11/06. **Refonte multi-maquettes (12/06)** : `MODELS_3D` = slug → **liste** `{label, src}` ; un sélecteur (chips `.mv-chip`) apparaît si plusieurs maquettes. Le panneau vit dans `<template id="tplMaquette">` HORS de `<main>` et `initMaquette()` est appelé APRÈS le montage de la vue : indispensable car `AxionAnalysis.mount()` (chantiers en études, ex. Gueules Cassées) remplace tout le `innerHTML` des onglets/panneaux — l'onglet est alors recréé dynamiquement. **Gueules Cassées ajouté le 12/06** : `maquette-gueules-cassees-chateau.glb` (1,5 Mo, 917 k triangles, « Le Château », modélisation en cours — mail O. Pénarette 11/06, fichier `Chateau.glb`), slug `gueules-cassees`. ⚠️ Ce GLB est compressé **Draco** (`KHR_draco_mesh_compression`), pas meshopt — décodeur par défaut de model-viewer, rien à configurer. D'autres maquettes GC sont annoncées : les ajouter à la liste du slug, le sélecteur apparaîtra tout seul. **Signature Cadence** dans la visionneuse : emblème « A » blanc en grand filigrane derrière la maquette (opacité 9 %) + logotype blanc en haut à gauche — fichiers `assets/img/logo-cadence-emblem-white.png` / `logo-cadence-white.png` (issus de `Documents\Claude\FRANCILIENNE\LOGOS\LOGO-CADENCE\PNG\HD`).
...
- **Roue du dashboard lisible + page « Qui sommes-nous »** (`app.html` + `cadence.html` — 10/06) : les cercles de la roue des métiers du tableau de bord ont été agrandis (nœuds r 22→31, libellés 9,5→13 px, sous-titres 6,5→8 px, contraste renforcé, roue élargie à 50 % du héro). Le **centre de la roue est désormais cliquable** → nouvelle page `cadence.html` « Cadence Architectes — Qui sommes-nous ? » (charte AXION bleu nuit/or), inspirée de la page équipe du site Cadence (axion-sand-iota/equipe.html) : 5 associés, 10 collaborateurs, 8 compétences transverses, CTA contact. Portraits = hotlink `www.cadence-architectes.fr/wp-content` (mêmes sources que le site officiel). Les hubs des roues `metiers.html` et `entreprises.html` pointent aussi vers `cadence.html`.

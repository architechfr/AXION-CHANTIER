...
| ✅ 1 planning ↔ 1 projet (pas de planning orphelin) | 06/06 | Un planning appartient toujours à un projet (`project_id`), **1 seul planning courant par projet**. Le configurateur se lance **depuis un projet sans planning** (CTA MOE/admin), génère selon `project.status` (étude/DCE vs chantier), puis **attache** (snapshot + `planning_url`) sans toucher aux autres. Détail complet : **`ARCHITECTURE-PLANNING.md`**. |
